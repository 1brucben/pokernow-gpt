...
export class Bot {
      ...
      if (res.code == "success") {
        ...
        if (data.includes("action-signal")) {
          ...
          await this.updateHero(
            hand,
            convertToBBs(stack_size, this.game.getBigBlind()),
          );
          ...
          try {
            ...
            console.error(
              "[Bot] Error during turn, falling back to check/fold:",
              err,
            );
            try {
              await this.performBotAction(defaultCheckAction);
            } catch {
              await this.performBotAction(defaultFoldAction);
            }
          }
  ...
  private async pullAndProcessLogs(
    ...
    if (log.code === "success") {
      ...
      if (first_seat_number === undefined) {
        console.warn(
          `[Bot] Could not find seat for SB player ${this.table.getFirstSeatOrderId()}, defaulting to seat 1`,
        );
        first_seat_number = 1;
      }
  ...
  private async queryBotAction(
    ...
    if (retry_counter > retries) {
      if (await this.isValidBotAction(defaultCheckAction)) {
        console.log(
          `Failed to query bot action, exceeded the retry limit after ${retries} attempts. Defaulting to checking.`,
        );
        return defaultCheckAction;
      } else {
        console.log(
          `Failed to query bot action, exceeded the retry limit after ${retries} attempts. Defaulting to folding.`,
        );
  ...
  private async isValidBotAction(bot_action: BotAction): Promise<boolean> {
    ...
    if (
      ...
      valid_actions.includes(bot_action.action_str)
    ) {
      let res;
    ...
    switch (bot_action.action_str) {
      ...
      case "all-in":
        bet_size = convertToValue(
          this.game.getHero()!.getStackSize(),
          this.game.getBigBlind(),
        );
        console.log(
