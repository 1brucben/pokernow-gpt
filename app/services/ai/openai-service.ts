...
import {
  getPromptFromPlaystyle,
  parseResponse,
} from "../../helpers/ai-query-helper.ts";
  ...
  async query(input: string, prev_messages: AIMessage[]): Promise<AIResponse> {
    if (prev_messages.length > 0) {
      ...
      try {
        ...
        const playstyle_prompt =
          customPrompt && customPrompt.trim().length > 0
            ? customPrompt
            : getPromptFromPlaystyle(this.getPlaystyle());
        prev_messages = [
        ...
        console.log(err);
        prev_messages = [{ text_content: input, metadata: { role: "user" } }];
      }
    }
