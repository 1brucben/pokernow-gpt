...
export class LogService {
  ...
  private use_fetch: boolean = false;
  ...
  async initFetchMode(): Promise<void> {
    this.use_fetch = true;
  }
  ...
  async fetchData<D, E = Error>(
    before: string = "",
    after: string = "",
  ): Response<D, E> {
    const url = `https://www.pokernow.com/games/${this.game_id}/log?before_at=${before}&after_at=${after}&mm=false&v=2`;
    ...
    if (this.use_fetch) {
      try {
        ...
        return {
          code: "error",
          error: new Error(`Failed to fetch logs: ${err.message}`) as E,
        };
      }
    }
    ...
    try {
      ...
      const logs_str = await this.page.$eval(
        "body > pre",
        (pre) => pre.textContent,
      );
      return {
      ...
      return {
        code: "error",
        error: new Error(
          "Failed to retrieve the text content of the api call.",
        ) as E,
      };
    }
  }
