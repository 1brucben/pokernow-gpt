...
export class PuppeteerService {
  ...
  private connect_to_existing: boolean;
  private remote_debugging_port: number;
  ...
  constructor(
    ...
    connect_to_existing: boolean = false,
    remote_debugging_port: number = 9222,
  ) {
    this.default_timeout = default_timeout;
    this.headless_flag = headless_flag;
    this.connect_to_existing = connect_to_existing;
    this.remote_debugging_port = remote_debugging_port;
  }

  isConnectMode(): boolean {
    return this.connect_to_existing;
  }

  async init(): Promise<void> {
    if (this.connect_to_existing) {
      try {
        this.browser = await puppeteer.connect({
          browserURL: `http://localhost:${this.remote_debugging_port}`,
          defaultViewport: null,
        });
        // Find the PokerNow tab among open pages
        const pages = await this.browser.pages();
        const pokerPage = pages.find((p) =>
          p.url().includes("pokernow.com/games/"),
        );
        if (pokerPage) {
          this.page = pokerPage;
          console.log(`Connected to existing PokerNow tab: ${pokerPage.url()}`);
        } else {
          throw new Error(
            "No PokerNow game tab found. Please open a PokerNow game in your browser before starting the bot.",
          );
        }
      } catch (err: any) {
        if (err.message?.includes("No PokerNow game tab found")) {
          throw err;
        }
        throw new Error(
          `Failed to connect to browser on port ${this.remote_debugging_port}. ` +
            `Make sure Chrome is running with: --remote-debugging-port=${this.remote_debugging_port}\n` +
  ...
  async closeBrowser(): Promise<void> {
    if (this.connect_to_existing) {
      // In connect mode, disconnect without closing the user's browser
  ...
  getGameIdFromUrl(): string | null {
    const url = this.page.url();
    const match = url.match(/pokernow\.com\/games\/([a-zA-Z0-9_-]+)/);
  ...
  async getGameInfo<D, E = Error>(): Response<D, E> {
    var game_info;
    try {
      game_info = await this.page.$eval(
        ".game-infos > .blind-value-ctn > .blind-value",
  ...
  convertGameInfo(game_info: string): GameInfo {
    ...
    if (matches && matches.length == 4) {
      return {
        game_type: matches[1],
        big_blind: Number(matches[3]),
        small_blind: Number(matches[2]),
      };
    } else {
  ...
  async sendEnterTableRequest<D, E = Error>(
    name: string,
    stack_size: number,
  ): Response<D, E> {
    if (name.length < 2 || name.length > 14) {
      return {
        code: "error",
        error: new Error(
          "Player name must be betwen 2 and 14 characters long.",
        ) as E,
      };
    }
    try {
      await this.page.waitForSelector(".table-player-seat-button", {
        timeout: this.default_timeout * 4,
      });
      await this.page.$eval(".table-player-seat-button", (button: any) =>
        button.click(),
      );
    } catch (err) {
      return {
    ...
    await this.page.$eval(".selected > div > form > button", (button: any) =>
      button.click(),
    );
    try {
      await this.page.waitForSelector(".alert-1-buttons > button", {
        timeout: this.default_timeout,
      });
      await this.page.$eval(".alert-1-buttons > button", (button: any) =>
        button.click(),
      );
    } catch (err) {
      ...
      if (
        await this.page.$(
          ".selected > div > form > div:nth-child(1) > .error-message",
        )
      ) {
        message = "Player name must be unique to game.";
      }
      await this.page.$eval(".selected > button", (button: any) =>
        button.click(),
      );
      return {
        code: "error",
  ...
  async waitForTableEntry<D, E = Error>(): Response<D, E> {
    try {
      await this.page.waitForSelector(".you-player", {
        timeout: this.default_timeout * 120,
      });
    } catch (err) {
      return {
        code: "error",
  ...
  // otherwise, return
  async waitForNextHand<D, E = Error>(
    num_players: number,
    ...
    try {
      await this.page.waitForSelector(
        [".you-player > .waiting", ".you-player > .waiting-next-hand"].join(
          ",",
        ),
        { timeout: this.default_timeout },
      );
    ...
    try {
      await this.page.waitForSelector(
        ...
        ),
        {
          hidden: true,
          timeout:
            computeTimeout(num_players, max_turn_length, 4) * 5 +
  ...
  async getNumPlayers<D, E = Error>(): Response<D, E> {
    try {
      ...
      const table_players_count = (await this.page.$$eval(
        ".table-player",
        (divs: any) => divs.length,
      )) as number;
      const table_player_status_count = (await this.page.$$eval(
        ".table-player-status-icon",
        (divs: any) => divs.length,
      )) as number;
  ...
  async waitForBotTurnOrWinner<D, E = Error>(
    num_players: number,
    max_turn_length: number,
  ): Response<D, E> {
    try {
      const el = await this.page.waitForSelector(
        [".action-signal", ".table-player.winner"].join(","),
        {
          timeout:
            computeTimeout(num_players, max_turn_length, 4) * 5 +
            this.default_timeout,
  ...
  async waitForBotTurnEnd<D, E = Error>(): Response<D, E> {
    try {
      await this.page.waitForSelector(".action-signal", {
        hidden: true,
  ...
  async getPotSize<D, E = Error>(): Response<D, E> {
    try {
      ...
      const pot_size_str = await this.page.$eval(
        ".table > .table-pot-size > .main-value",
        (p: any) => p.textContent,
      );
      return {
        code: "success",
  ...
  async getHand<D, E = Error>(): Response<D, E> {
    try {
      const cards_div = await this.page.$$(
        ".you-player > .table-player-cards > div",
      );
      let cards: string[] = [];
      for (const card_div of cards_div) {
        const card_value = await card_div.$eval(
          ".value",
          (span: any) => span.textContent,
        );
        const sub_suit_letter = await card_div.$eval(
          ".sub-suit",
          (span: any) => span.textContent,
        );
  ...
  async cancelUnnecessaryFold<D, E = Error>(): Response<D, E> {
    ...
    try {
      await this.page.waitForSelector(".alert-1", {
        timeout: this.default_timeout,
      });
      ...
      return {
        code: "error",
        error: new Error(
          "No option to cancel unnecessary fold available.",
        ) as E,
      };
    }
    return {
  ...
  async betOrRaise<D, E = Error>(bet_amount: number): Response<D, E> {
    try {
      const bet_action = await this.page.$eval(
        ".game-decisions-ctn > .action-buttons > .raise",
  ...
  async getCurrentBet<D, E = Error>(): Response<D, E> {
    try {
      ...
      const current_bet = await this.page.evaluate(
        (el: any) => (isNaN(el.textContent) ? "0" : el.textContent),
        el,
      );
      return {
        code: "success",
  ...
  async waitForHandEnd<D, E = Error>(): Response<D, E> {
    try {
      await this.page.waitForSelector(".table-player.winner", {
        hidden: true,
